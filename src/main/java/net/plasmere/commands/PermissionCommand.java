        ...
        LiteralArgumentBuilder<ServerCommandSource> builder = CommandManager
                ...
                .then(
                        CommandManager.argument("target", EntityArgumentType.player())
                                                ...
                                                .then(
                                                        CommandManager.argument("permission", MessageArgumentType.message())
                                                                .executes(context -> {
                                                                    ...
                                                                    TacoRevamped.getConfiguration().getPermissions().addPermission(playerEntity.getUuidAsString(), permission);
                                                ...
                                                .then(
                                                        CommandManager.argument("permission", MessageArgumentType.message())
                                                                .executes(context -> {
                                                                    String permission = MessageArgumentType.getMessage(context, "permission").asString();
                                                                    ServerPlayerEntity playerEntity = EntityArgumentType.getPlayers(context, "target").iterator().next();
                                                                    TacoRevamped.getConfiguration().getPermissions().removePermission(playerEntity.getUuidAsString(), permission);
                                ...
                                .executes(context -> {
                                    ServerPlayerEntity playerEntity = EntityArgumentType.getPlayers(context, "target").iterator().next();
                                    Set<String> permissions = TacoRevamped.getConfiguration().getPermissions().getPermissions().get(playerEntity.getUuidAsString());
                                    if (permissions == null) {
