...
public class VoteCommand {
    private static int run(CommandContext<ServerCommandSource> context) throws CommandSyntaxException {
        try {
            ...
            self.sendMessage(Utils.codedText("&3VOTE &8>> &bIf you wish to vote for the server, then click this link&8: &6http://vote.plasmere.net"), false);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return 0;
    ...
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        LiteralArgumentBuilder<ServerCommandSource> literalArgumentBuilder = LiteralArgumentBuilder.literal("vote");
        literalArgumentBuilder.then(CommandManager.argument("player", EntityArgumentType.player())
                .requires((context) -> TacoRevamped.getConfiguration().getPermissions().checkPermissions(context, "vote.others") || context.hasPermissionLevel(2))
                .executes(VoteCommand::runOther)
        );
        literalArgumentBuilder
//                .requires((context) -> {
//                    try {
//                        PlayerEntity playerEntity = context.getPlayer();
//                        return SettingsManager.canUseCommand(playerEntity, SettingsManager.Command.VOTE);
